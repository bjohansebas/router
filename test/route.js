  ...
  describe(`Router - ${type}`, function () {
    describe('.route(path)', function () {
      ...
      it('should route without method', function (done) {
        if (type === 'http2') {
          // HTTP2 requires a method to be defined, so this test is not applicable
          this.skip()
        }
        ...
        it('should invoke next handler', function (done) {
          ...
          route.get(function handle (req, res, next) {
            if (type === 'http2') {
              ...
              res.headers = { ...res.headers, 'x-next': 'route' }
            } else {
...
function helloWorld (req, res) {
  if (req.httpVersion === '2.0') {
    res.stream.respond({ ':status': 200, 'content-type': 'text/plain', ...res.headers })
...
function sendParams (req, res) {
  if (req.httpVersion === '2.0') {
    res.stream.respond({ ':status': 200, 'content-type': 'application/json', ...res.headers })
    res.stream.end(JSON.stringify(req.params))
  } else {
    res.statusCode = 200
