...
function runTestSuite (type) {
  const { createServer, request, rawrequest } = utils.helperServer(type)
  ...
  describe(`Router - ${type}`, function () {
    ...
    describe('.all(path, fn)', function () {
      ...
      it('should not stack overflow with many registered routes', function (done) {
        this.timeout(6000) // long-running test
        ...
        it('should invoke next handler', function (done) {
          ...
          router.use(function handle (req, res, next) {
            if (req.httpVersion === '2.0') {
              res.headers = { ['x-next']: 'route', ...res.headers}
            }
            else {
    ...
    describe('.use(path, ...fn)', function () {
      ...
      describe('req.baseUrl', function () {
        ...
        it('should be altered correctly', function (done) {
          ...
          sub3.get('/zed', setsawBase(1), function (req, res, next) {
            if ( req.httpVersion === '2.0' ) {
              res.stream.respond({ ':status': 200, ...res.headers })
            } 
...
function setsaw (num) {
  ...
  return function saw (req, res, next) {
    if (req.httpVersion === '2.0') {
      res.headers = {[name]: req.method + ' ' + req.url , ...res.headers}
    } else {
...
function setsawBase (num) {
  ...
  return function sawBase (req, res, next) {
    if (req.httpVersion === '2.0') {
      res.headers = {...res.headers, [name]: String(req.baseUrl)}
    } else {
